...
function start_github_webhook(http_ip=config["DEFAULT_HTTP_IP"], http_port=config["DEFAULT_HTTP_PORT"])
    auth = get_jwt_auth()
    listener = GitHub.CommentListener(comment_handler, r"`${TRIGGER}\(.*?\)`"; check_collab=false, auth=auth, secret=config["GITHUB_SECRET"])
    httpsock[] = Sockets.listen(IPv4(http_ip), http_port)
    GitHub.run(listener, httpsock[], IPv4(http_ip), http_port)
end
...
function processor()
    ...
    while isopen(event_queue)
        try
            handle_register_events(take!(event_queue))
            backoff = 0
        catch ex
            if isa(ex, InvalidStateException) && (ex.state == :closed)
                @warn("Processor channel closed. Exiting...")
            else
                @error("Recovering from unknown exception", ex, backoff)
                sleep(backoff)
...
function monitor()
    while isopen(event_queue)
        ...
        if isfile(SERVER_STOP_FILE)
            @warn("Server stop requested.")
            close(event_queue)
            close(httpsock[])
            rm(SERVER_STOP_FILE; force=true)
        end
    end
end
